...
export const deleteUsuario = async (req, res) => {
  ...
  try {
    ...
    if (result.affectedRows === 1) {
      res.send(
        '<script>alert("Usuario eliminado exitosamente"); window.location="/usuario";</script>'
      );
    } else {
      res.redirect("/usuario");
    }
...
export const createUsuariosPersonal = async (req, res) => {
  ...
  try {
    ...
    // Registro de log
    const usuario = req.session.usuario;
    let crearLog = `Creación de usuario de personal: ${newUsuario.username} con correo ${newUsuario.email}, realizada por el supervisor: ${
      usuario.username
    } a las ${new Date().toLocaleString()}`;
    pool.query("INSERT INTO reportes (contenido) values (?)", [crearLog]);
